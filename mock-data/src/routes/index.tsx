...
import HomePage, { fetch as HomeFetch } from '@/pages/Home';
...
function PageLoader({ Component, fetch }: PageLoaderProps<unknown>) {
...
export const routeConfig: RouteConfigItem<unknown>[] = [
  { path: '/', Component: HomePage, fetch: HomeFetch },
  {
    // 404
    path: '*',
...
export function RouteContainer() {
  return useRoutes(
    routeConfig.map(({ path, Component, fetch }) => {
      return {
        path,
        element: <PageLoader Component={Component} fetch={fetch} />,
      };
    }),
  );
}
